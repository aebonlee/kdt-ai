...
create table if not exists public.kdt_evaluations (
  ...
  class_no     int,
  profile_id   uuid,                     -- kdt_profiles 연결(등록 학생인 경우)
...
create index if not exists kdt_evaluations_subject_idx
  on public.kdt_evaluations (subject_id);

alter table public.kdt_evaluations enable row level security;

-- 관리자(강사)만 접근
drop policy if exists "evaluations_admin_all" on public.kdt_evaluations;
create policy "evaluations_admin_all" on public.kdt_evaluations
  for all using (public.kdt_is_admin()) with check (public.kdt_is_admin());
