...
export const details = {
  ...
  'python-1': {
    topics: [
      { h: '환경과 기본 문법', items: ['Jupyter/Colab 셀 실행·마크다운', '변수·자료형(int/float/str/bool)', '연산자·문자열 포매팅(f-string)', '입력·출력·주석'] },
      { h: '자료구조', items: ['리스트·튜플·딕셔너리·집합', '인덱싱·슬라이싱', '컴프리헨션(list/dict)', '가변 vs 불변'] },
      { h: '제어·함수·NumPy', items: ['조건문·반복문·enumerate/zip', '함수·매개변수·반환·예외 처리', 'NumPy 배열 생성·연산', '브로드캐스팅·집계 함수'] },
    ],
    labs: [
      { title: 'Lab1. 자료구조 미니 문제', steps: ['리스트로 점수 통계(합/평균/최대)', '딕셔너리로 단어 빈도수 집계', '집합으로 중복 제거·교집합'] },
      { title: 'Lab2. NumPy 벡터 연산', steps: ['반복문 합 vs np.sum 속도 비교', '배열 정규화(평균0·분산1)', '조건 인덱싱으로 이상치 추출'] },
    ],
    homework: ['주제 데이터(csv) 1개를 골라 자료구조로 요약 통계 계산', '반복문 버전과 NumPy 버전 코드를 함께 제출'],
  },
  'python-2': {
    topics: [
      { h: 'Pandas 기초', items: ['Series/DataFrame 생성·구조', 'read_csv·head·info·describe', 'loc/iloc 인덱싱·조건 선택', '컬럼 추가·삭제·이름 변경'] },
      { h: '정제와 변환', items: ['결측치 dropna/fillna', '자료형 변환·중복 제거', 'apply/map 으로 파생 컬럼', '문자열·날짜 처리'] },
      { h: '집계와 시각화', items: ['groupby 집계·agg', 'merge·concat·pivot_table', 'matplotlib 기본 차트', 'seaborn 분포·상관 히트맵'] },
    ],
    labs: [
      { title: 'Lab1. 데이터 정제', steps: ['결측치·이상치 진단', '규칙 정해 처리(대체/제거)', '정제 전후 통계 비교'] },
      { title: 'Lab2. 그룹 집계·시각화', steps: ['범주별 평균/건수 groupby', 'pivot_table 로 교차표', '막대·박스플롯으로 시각화'] },
    ],
    homework: ['선택 데이터로 EDA 노트북 작성(결측 처리→집계→차트 3종)', '발견한 인사이트 3가지를 마크다운으로 정리'],
  },

  // ── 웹 서비스 개발 mini-Project ──
  'webproject-1': {
    topics: [
      { h: '주제·요구사항', items: ['문제 정의·타깃 사용자', '핵심 가치(MVP) 선정', '사용자 시나리오 작성', '기능 목록·우선순위'] },
      { h: '설계', items: ['화면 흐름·와이어프레임', '데이터 모델(엔티티/관계)', 'API 엔드포인트 정의', '화면-API 매핑'] },
      { h: '셋업', items: ['기술 스택 선정 근거', '리포지토리·브랜치 전략', '작업 분담·일정(WBS)', '협업 규칙(커밋·이슈)'] },
    ],
    labs: [
      { title: 'Lab1. 와이어프레임', steps: ['핵심 3화면 스케치', '화면 전환 흐름 표시', '필요한 데이터 항목 표기'] },
      { title: 'Lab2. API 설계서', steps: ['엔티티·필드 정의', 'CRUD 엔드포인트 표 작성', '요청/응답 예시 명세'] },
    ],
    homework: ['팀별 기획서(문제·MVP·시나리오) 1장 정리', '와이어프레임 + API 설계서 초안 제출'],
  },
  'webproject-2': {
    topics: [
      { h: '구조 구현', items: ['공통 레이아웃·헤더/푸터', '라우팅·페이지 구성', '재사용 컴포넌트 분리', '상태 관리 구조'] },
      { h: '기능 구현', items: ['목록·상세·폼 화면', 'API 연동(fetch/axios)', '로딩·에러·빈 상태 처리', '폼 유효성 검사'] },
      { h: '협업', items: ['컴포넌트 단위 분담', '브랜치·PR·코드리뷰', '공통 컴포넌트 공유', '중간 통합 점검'] },
    ],
    labs: [
      { title: 'Lab1. 목록-상세 연동', steps: ['목록 API 연동·렌더', '상세 라우팅·파라미터', '로딩·에러 UI 처리'] },
      { title: 'Lab2. 입력 폼', steps: ['폼 상태·바인딩', '제출 전 유효성 검사', '성공·실패 피드백'] },
    ],
    homework: ['담당 화면 1개를 동작 가능한 수준으로 구현', '미완 기능·이슈를 보드에 정리'],
  },
  'webproject-3': {
    topics: [
      { h: '통합·안정화', items: ['기능 통합·충돌 해결', '버그 수정·예외 보강', 'UI/UX 다듬기', '반응형·접근성 점검'] },
      { h: '배포', items: ['환경변수 분리(.env)', '프로덕션 빌드', '정적 호스팅 배포', 'SPA 라우팅 폴백'] },
      { h: '발표', items: ['데모 시나리오 구성', '발표 자료·역할 분담', '한계·개선점 정리', '상호 피드백·회고'] },
    ],
    labs: [
      { title: 'Lab1. QA 체크리스트', steps: ['핵심 흐름 시나리오 점검', '버그 기록·우선순위', '수정 후 회귀 확인'] },
      { title: 'Lab2. 배포', steps: ['빌드·환경변수 설정', '호스팅에 배포', '배포본 동작 검증'] },
    ],
    homework: ['배포 URL + 리포 링크 제출', '회고(잘된 점·아쉬운 점·다음 개선) 작성'],
  },

  // ── 실전 Feature Engineering ──
  'feature-1': {
    topics: [
      { h: '수치형 처리', items: ['결측치 진단·대체(평균/중앙/KNN)', '이상치 탐지(IQR/z-score)', '스케일링(Standard/MinMax/Robust)', '로그·박스콕스 변환'] },
      { h: '범주·파생', items: ['원-핫·라벨·타깃 인코딩', '날짜에서 요일·월·주말 추출', '텍스트 길이·키워드 플래그', '상호작용·구간화(binning)'] },
      { h: '선택·축소', items: ['상관·분산 기반 제거', '모델 중요도(permutation)', 'PCA 차원 축소', '데이터 누수 차단(fit은 train만)'] },
    ],
    labs: [
      { title: 'Lab1. 인코딩 비교', steps: ['원-핫 vs 타깃 인코딩 적용', '교차검증 내 타깃 인코딩으로 누수 방지', '성능 차이 비교'] },
      { title: 'Lab2. 파생 피처 효과', steps: ['날짜·상호작용 피처 생성', '피처 추가 전후 성능 측정', '중요도 상위 피처 해석'] },
    ],
    homework: ['데이터셋에 피처 5종 이상 생성·문서화', '피처 추가 전후 검증 점수 비교표 제출'],
  },

  // ── 모델 개발 및 최적화 ──
  'modeldev-1': {
    topics: [
      { h: '문제·모델 선택', items: ['분류/회귀 문제 정의', '평가지표 선택 기준', '모델 후보(선형·트리·부스팅)', '베이스라인 수립'] },
      { h: '검증 설계', items: ['train/valid/test 분할', 'K-fold 교차검증', '계층적 분할(불균형)', '데이터 누수 점검'] },
      { h: '파이프라인', items: ['전처리+모델 Pipeline', 'ColumnTransformer', '재현성(seed) 관리', '결과 로깅·비교'] },
    ],
    labs: [
      { title: 'Lab1. 베이스라인', steps: ['더미·선형 모델 학습', '교차검증 점수 측정', '혼동행렬/잔차 확인'] },
      { title: 'Lab2. 모델 비교', steps: ['3개 모델 동일 검증', '점수·학습시간 비교표', '최적 후보 선정'] },
    ],
    homework: ['베이스라인 + 모델 2종 교차검증 결과 정리', '선정 모델과 근거를 노트로 제출'],
  },
  'modeldev-2': {
    topics: [
      { h: '하이퍼파라미터 튜닝', items: ['Grid/Random Search', 'Bayesian(Optuna) 탐색', '탐색 공간·예산 설계', 'CV와 결합한 튜닝'] },
      { h: '과적합 제어', items: ['규제(L1/L2)·드롭아웃', '조기종료·학습곡선', '데이터 증강·노이즈', '편향-분산 트레이드오프'] },
      { h: '앙상블·경량화', items: ['배깅·부스팅·스태킹', '소프트/하드 보팅', '특성 중요도 해석', '가지치기·양자화로 경량화'] },
    ],
    labs: [
      { title: 'Lab1. Optuna 튜닝', steps: ['탐색 공간 정의', 'study 실행·best params', '튜닝 전후 성능 비교'] },
      { title: 'Lab2. 앙상블', steps: ['상위 모델 보팅/스태킹', '단일 vs 앙상블 비교', '안정성(분산) 점검'] },
    ],
    homework: ['튜닝으로 베이스라인 대비 성능 개선 보고', '최종 모델 카드(설정·점수·한계) 작성'],
  },
