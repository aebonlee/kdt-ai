...
//
// ⚠️ 이 목록은 파일 "링크"(드라이브 파일ID)만 담습니다. ID 자체는 빌드 시
//    공개 JS 번들에 포함되므로, 실제 보호는 "드라이브 공유설정"이 담당합니다.
//    → 이 폴더/파일은 소유자(대표) 전용(restricted)으로 잠근다.
//      권한 없는 사람은 ID를 알아도 preview/다운로드가 열리지 않는다.
//    → 강사·실습교수 공유는 여기서 하지 않는다. skala 리포에서 공개한
//      별도 드라이브를 사용한다(접근 대상이 다르므로 자료도 분리).
//    관리자 UI 게이트(RequireAdmin)는 화면 노출만 막을 뿐 번들 추출은
//    못 막으므로, 드라이브 restricted가 최종 방어선이다.
//
// 원본 폴더: SKALA 4기 실습교수 자료 (docs) — 소유자 전용
...
export const docsGroups = [
  {
    id: 'timetable',
    title: '★ 반별 강사 시간표',
    desc: '18개 분반 강사 배정 확정본과 캠퍼스별 시간표 양식',
    files: [
  ...
  {
    id: 'textbook',
    title: '교육생 교재 (PDF ver.)',
    desc: '교육생에게 배포되는 과목별 교재',
    files: [
  ...
  },
  {
    id: 'eval',
    title: '종합실습 평가',
    desc: '과목별 평가 양식·기준·작성예시와 실습 시행 기록',
    files: [
  ...
  {
    id: 'slack',
    title: '실습교수 배포자료 (슬랙 공유)',
    desc: '수업 중 슬랙으로 공유된 실습 자료 — 기대답안·실습 데이터·환경 구축 스크립트',
    files: [
