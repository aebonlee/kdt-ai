...
export default function CurriculumFlow() {
  ...
  return (
    <svg viewBox="0 0 1180 470" width="100%" role="img"
      ...
      {/* 상단 단계 구분 화살표 */}
      <line x1="12" y1="44" x2="1000" y2="44" stroke="#2a3040" strokeWidth="1.5" markerEnd="url(#cf-arr)" />
      <line x1="1030" y1="44" x2="1168" y2="44" stroke="#2a3040" strokeWidth="1.5" markerEnd="url(#cf-arr)" />
      <defs>
        <marker id="cf-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
          <path d="M0,0 L6,3 L0,6" fill="none" stroke="#2a3040" strokeWidth="1.2" />
        </marker>
      </defs>
      ...
      <line x1="1012" y1="16" x2="1012" y2="418" stroke="#222634" strokeWidth="1.5" strokeDasharray="4 4" />

      {/* 계단식 과목 블록 */}
      {courses.map((c) => (
        <g key={c.t2}>
          <rect x={c.x} y={c.y} width={c.w} height="56" rx="10" fill="url(#cf-course)" stroke="#2b4a5f" />
          <text x={c.x + c.w / 2} y={c.y + 23} textAnchor="middle" fill="#9aa4bd" fontSize="12.5">{c.t1}</text>
          <text x={c.x + c.w / 2} y={c.y + 42} textAnchor="middle" fill="#e8eaf0" fontSize="14.5" fontWeight="800">{c.t2}</text>
        </g>
      ))}
      ...
      {minis.map((m) => (
        <g key={m.label}>
          <rect x={m.x} y={m.y} width={m.w} height="56" rx="10" fill="#12141c" stroke="#2a3040" strokeDasharray="5 4" />
          <rect x={m.x + m.w / 2 - 42} y={m.y + 9} width="84" height="18" rx="9" fill="#1b3a3f" />
          <text x={m.x + m.w / 2} y={m.y + 22} textAnchor="middle" fill="#22d3ee" fontSize="10.5" fontWeight="700">Mini-project</text>
          <text x={m.x + m.w / 2} y={m.y + 44} textAnchor="middle" fill="#c4c9d8" fontSize="13.5" fontWeight="700">{m.label}</text>
        </g>
      ))}
      ...
      {/* 하단 상시 트랙 */}
      <rect x="12" y="392" width="1156" height="52" rx="10" fill="url(#cf-lime)" opacity="0.14" stroke="#2b4a5f" />
      {rail.map((r, i) => {
        const seg = 1156 / 4
        const cx = 12 + seg * i + seg / 2
        if (i > 0) return (
          <g key={r.label}>
            <line x1={12 + seg * i} y1="400" x2={12 + seg * i} y2="436" stroke="#2a3040" strokeWidth="1" />
            <text x={cx} y="423" textAnchor="middle" fill="#c4c9d8" fontSize="13.5" fontWeight="700">{r.icon} {r.label}</text>
          </g>
        )
        return <text key={r.label} x={cx} y="423" textAnchor="middle" fill="#c4c9d8" fontSize="13.5" fontWeight="700">{r.icon} {r.label}</text>
      })}
    </svg>
  )
}
