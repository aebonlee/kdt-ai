...
## 병행 작업 스트림: 울산 4반 성적처리 (2026-07-16~)
사이트 개발과 별개로 **슬랙 제출 → 엑셀 채점** 작업이 진행 중. 정본 인수인계는
`/Volumes/aebon - 데이터/SKALA-4/CLAUDE.md` (채널·스레드 TS, P/F 점수화 규칙, 우수자 배경색 기준, 미결사항).
- 대상: `/Volumes/aebon - 데이터/SKALA-4/실습 시행_울산_파이썬_RAW.xlsx` (이 리포와 무관 — 커밋 금지)
- 슬랙은 claude.ai Slack 커넥터로 읽음(로컬 CLI 세션엔 커넥터가 없을 수 있음 — 그 경우 성적처리는 웹/데스크톱 세션에서)
- 사이트에 평가 관련 기능·콘텐츠를 넣을 때 이 채점 기준(주석20/간결35/예외35/납기10)과 용어를 맞출 것
