...
create policy "evaluations_admin_all" on kdt_evaluations
  ...
  with check (kdt_is_admin());                   -- 쓸 때도: 관리자만`,note:"성적처럼 민감한 데이터는 서버(RLS)가 지켜야 한다 — 프런트에서 화면만 가리는 것은 보안이 아니다. jsonb 컬럼은 평가 항목이 과목마다 달라도 테이블 구조를 바꾸지 않게 해 주는 실전 요령이다."},{title:"React에서 Supabase CRUD — 조회 필터와 업서트 패턴",lang:"javascript",code:`// 이 사이트의 평가 화면이 실제로 쓰는 데이터 연동 패턴(요약본)
