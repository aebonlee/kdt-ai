...
초보자는 우선 'ref 만 쓴다'고 외워도 충분합니다.`},{h:"2교시 — Vite로 시작하는 이유와 개발환경 한 장 정리",body:`강의 흐름: 먼저 '왜 손으로 index.html·script 태그를 만들지 않고 Vite를 쓰나'를 묻습니다. 옛날에는 자바스크립트 파일을 여러 개 만들고 순서대로 script 태그로 끼워 넣어야 했고, 라이브러리 하나 추가할 때마다 CDN 주소를 붙였습니다. Vite는 이 모든 잡일을 대신 해 주는 '개발 도구 세트'입니다.
...
왜 중요한가: 회원가입·검색·할 일 추가 등 모든 입력 화면의 뼈대가 v-model 하나입니다. 입력창에 v-model='name'을 걸고 바로 아래 {{ name }}을 두어, 타이핑이 즉시 반영되는 '거울 효과'를 눈으로 확인시키는 것이 이 교시의 핵심 실습입니다. 8교시 미니 실습(카운터·할 일 목록)은 오늘 배운 ref+디렉티브+이벤트+v-model을 한 화면에 모으는 종합 연습이라는 점을 예고하며 마무리합니다.`}]},realCodes:[{title:"App.vue — 반응형 할 일 목록 전체",lang:"vue",code:`<script setup>
...
왜 중요한가: 같은 로직을 복사-붙여넣기로 여러 컴포넌트에 흩뿌리면 나중에 한 곳만 고쳐도 나머지가 깨집니다. 컴포저블로 한 번 만들어 여러 곳에서 부르면 '한 곳만 고치면 전부 반영'됩니다. 어제 만든 입력 로직을 useInput 컴포저블로 빼내 두 개의 다른 컴포넌트에서 재사용해 보이는 것으로 마무리하고, 8교시 미니 실습(목록·아이템 분리)으로 자연스럽게 넘어갑니다.`}]},realCodes:[{title:"UserItem.vue — props·emit·slot 을 쓰는 자식 컴포넌트",lang:"vue",code:`<script setup>
...
왜 중요한가: 실라버스의 '.env 환경 분리' 목표에 직결되고, 6교시 미니 SPA·7교시 배포로 그대로 이어집니다. API 주소를 .env로 빼서 개발/배포 주소를 한 줄 바꾸는 것만으로 전환되는 구조를 만들어 보이는 것이 핵심 실습입니다.`},{h:"UI 라이브러리 Element Plus로 화면 생산성 높이기 (실라버스 8단원)",body:`강의 흐름: 버튼·입력창·테이블·다이얼로그를 매번 CSS로 직접 예쁘게 만들려면 시간이 많이 듭니다. UI 라이브러리는 '검증된 공통 컴포넌트 모음'으로, 가져다 쓰기만 하면 일관된 디자인이 완성됩니다. Vue 생태계에서 널리 쓰는 Element Plus를 예로 듭니다.
...
왜 중요한가: 실라버스의 'UI 라이브러리 이식·공통 컴포넌트 규격 활용으로 생산성 향상' 목표가 이것입니다. 직접 CSS로 만든 폼과, el-form으로 5분 만에 만든 폼을 나란히 보여 주며 생산성 차이를 체감시킵니다. el-form에 el-input 두 개와 el-button을 배치하고, 입력 결과를 el-table로 뿌리는 미니 예제로 이어집니다.`},{h:"Modern JavaScript(ES6+) 핵심 문법 — Vue 코드가 이렇게 짧은 이유 (실라버스 9단원)",body:`강의 흐름: 지금까지 본 Vue 예제에 이미 화살표 함수·구조분해·전개 연산자가 잔뜩 들어 있었습니다. 이 교시는 '왜 그렇게 썼는지'를 되짚어 최신 자바스크립트 문법을 정리하는 시간입니다. 문법 자체가 목적이 아니라, Vue 코드를 읽고 짧게 쓰기 위한 도구로 접근합니다.
...
왜 중요한가: 실라버스의 'ES6+ 고급 명세 마스터로 최적화된 프론트엔드 코딩 역량'에 해당합니다. 같은 로직을 옛날 문법과 ES6+로 나란히 써 비교하면 왜 현대 코드가 짧고 안전한지 한눈에 보입니다. props 구조분해·map 렌더링·옵셔널 체이닝 세 가지를 실제 Vue 컴포넌트에 적용해 보는 것으로 마무리합니다.`},{h:"ESLint·Prettier로 코드 품질 지키기 (실라버스 10단원, 빌드 앞단)",body:`강의 흐름: 팀으로 개발하면 사람마다 들여쓰기·따옴표·세미콜론 습관이 달라 코드가 뒤죽박죽이 되고, 사소한 실수(선언만 하고 안 쓴 변수, 오타 난 함수명)가 버그로 이어집니다. 이 두 도구가 그 문제를 자동으로 잡아 줍니다. 역할이 다르므로 짝으로 씁니다.
...
왜 중요한가: 실라버스의 'ESLint·Prettier를 통한 코드 품질 관리' 목표이자, 배포 전 마지막 점검 단계입니다. 일부러 들여쓰기를 흩뜨리고 안 쓰는 변수를 넣은 코드를 저장했을 때 Prettier가 정리하고 ESLint가 경고하는 장면을 직접 보여 주면 두 도구의 차이가 명확해집니다. 이 정리가 끝난 깨끗한 코드를 npm run build로 압축해 dist를 만드는 7·8교시로 이어집니다.`}]},realCodes:[{title:"api.js + PostList.vue — API 연동 미니 SPA",lang:"vue",code:`<!-- ===== src/api.js (별도 파일) =====
